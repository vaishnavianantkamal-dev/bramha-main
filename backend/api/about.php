...
try {
    ...
    $result = $conn->query("SELECT * FROM about_section WHERE is_active = 1 LIMIT 1");
    if (!$result) throw new Exception("Query failed: " . $conn->error);

    if ($result->num_rows === 0) {
        http_response_code(200);
        echo json_encode(["success"=>true,"data"=>null,"message"=>"No about section found"]);
        exit();
    }
    ...
    if (!empty($row['stats'])) {
        ...
        $row['stats'] = (json_last_error() === JSON_ERROR_NONE) ? $decoded : [];
    } else {
        $row['stats'] = [];
    }

    http_response_code(200);
    echo json_encode(["success"=>true,"message"=>"About section fetched","data"=>$row], JSON_PRETTY_PRINT);
} catch (Exception $e) {
    http_response_code(500);
    echo json_encode(["success"=>false,"message"=>"Error fetching about section","error"=>$e->getMessage()], JSON_PRETTY_PRINT);
}
