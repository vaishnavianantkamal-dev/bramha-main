...
/**
 * Placement FAQs API
 * GET /backend/api/placement-faqs.php
 */
ob_start();
...
try {
    ...
    $result = $conn->query("SELECT id, question, answer, category, display_order FROM placement_faqs WHERE is_active = 1 ORDER BY display_order ASC");
    ...
    echo json_encode(["success"=>true,"message"=>"Placement FAQs fetched","data"=>$faqs,"count"=>count($faqs)], JSON_PRETTY_PRINT);
} catch (Exception $e) {
    http_response_code(500);
    echo json_encode(["success"=>false,"message"=>"Error fetching placement FAQs","error"=>$e->getMessage()], JSON_PRETTY_PRINT);
}
