 ...
 * GET /backend/api/hero-slides.php
 */
ob_start();
...
try {
    ...
    $sql = "SELECT id, image, tag, headline, highlight, subtitle, display_order
            FROM hero_slides WHERE is_active = 1 ORDER BY display_order ASC";
    ...
    echo json_encode(["success"=>true,"message"=>"Hero slides fetched","data"=>$slides,"count"=>count($slides)], JSON_PRETTY_PRINT);
} catch (Exception $e) {
    http_response_code(500);
    echo json_encode(["success"=>false,"message"=>"Error fetching hero slides","error"=>$e->getMessage()], JSON_PRETTY_PRINT);
}
