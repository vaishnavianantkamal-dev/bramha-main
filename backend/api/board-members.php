...
/**
 * Board Members API
 * GET /backend/api/board-members.php
 */
ob_start();
...
try {
    ...
    $result = $conn->query("SELECT id, name, role, image, bio, email, phone, display_order FROM board_members WHERE is_active = 1 ORDER BY display_order ASC");
    ...
    echo json_encode(["success"=>true,"message"=>"Board members fetched","data"=>$members,"count"=>count($members)], JSON_PRETTY_PRINT);
} catch (Exception $e) {
    http_response_code(500);
    echo json_encode(["success"=>false,"message"=>"Error fetching board members","error"=>$e->getMessage()], JSON_PRETTY_PRINT);
}
