 ...
 * GET /backend/api/gallery-categories.php
 */
ob_start();
...
try {
    ...
    $result = $conn->query("SELECT id, name, slug, description, cover_image, display_order FROM gallery_categories WHERE is_active = 1 ORDER BY display_order ASC");
    ...
    echo json_encode(["success"=>true,"message"=>"Gallery categories fetched","data"=>$categories,"count"=>count($categories)], JSON_PRETTY_PRINT);
} catch (Exception $e) {
    http_response_code(500);
    echo json_encode(["success"=>false,"message"=>"Error fetching gallery categories","error"=>$e->getMessage()], JSON_PRETTY_PRINT);
}
